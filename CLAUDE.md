...
Software workflow (`host-lint`): commit and push inside the `host-lint/` worktree first, then record the new SHA as `.host-software`'s `pin` and push that host commit — the pin replaces the old submodule pointer (`call/0010`). The `template-agentic-host` submodule still uses the pointer-bump: commit and push inside it first, then commit the bumped pointer. Never push a host commit whose software pin or submodule pointer is unpushed. If a mandated push fails (no auth, no network), stop, report the unpushed commits to the user, and do not start dependent work.
